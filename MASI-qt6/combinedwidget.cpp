...
void CombinedWidget::setLabels(const QString &a, const QString &b, const QString &c, const QString &d, const QString &e, const QString &f ){
    labelA = a;
    labelB = e;
    labelC = f;
    verticalLabels = { ";", b, ";", c + " - ?" };
...
void CombinedWidget::paintEvent(QPaintEvent *event) {
    ...
    int baseX = widthMid - 80;
    int spacingX = 50;
    painter.drawText(baseX + 0 * spacingX, topY - 10, labelA);          // "1"
    painter.drawText(baseX + 1 * spacingX, topY - 10, ";");
    painter.drawText(baseX + 2 * spacingX, topY - 10, labelB);          // "B"
    painter.drawText(baseX + 3 * spacingX, topY - 10, ";");
    painter.drawText(baseX + 4 * spacingX, topY - 10, labelC + " - ?"); // "u - ?"

    // ---- Rysuj pionową linię pod "1" ----
    int verticalX = baseX;  // linia pionowa pod "1"
    int verticalTop = topY + 20;
