...
int main(int argc, char *argv[]) {
    ...
    qDebug() << "EEEEE";
    qDebug() << "Ścieżka do bazy danych:" << QDir::current().absoluteFilePath("moja_baza.db");
   ...
   // canvasLayout->setContentsMargins(10, 10, 10, 10);0
    ...
    auto *buttonview = new QPushButton("Zobacz dane");
    ...
    QObject::connect(buttonsave, &QPushButton::clicked, [&]() {
        ...
        if (dialog.exec() == QDialog::Accepted) {
            QString nazwa = dialog.getName();
            QString opis = dialog.getDescription();

            if (Database::insertRecord(nazwa, opis, val1, val2, valW, valA, valB, valU)) {
                QMessageBox::information(&window, "Sukces", "Dane zostały zapisane do bazy.");
            } else {
                QMessageBox::critical(&window, "Błąd", "Nie udało się zapisać danych do bazy.");
            }
        }
    });
